#!/usr/bin/env python3
import os
import sys
import time

import rclpy
from rclpy.node import Node
from std_msgs.msg import String

_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if _ROOT not in sys.path:
    sys.path.append(_ROOT)

from Frankenstein_Board.frankenstein_hw import FrankensteinSonarDriver


class FrankensteinSonarTestNode(Node):
    def __init__(self) -> None:
        super().__init__("frankenstein_sonar_test")
        self.declare_parameter("samples", 5)
        self.declare_parameter("sample_dt", 0.2)
        self.declare_parameter("max_mm", 5000)
        self.declare_parameter("command_topic", "/frankenstein/test_command")
        self.declare_parameter("result_topic", "/frankenstein/test_result")
        self.declare_parameter("auto_run", False)

        self._sonar = FrankensteinSonarDriver()
        self._samples = int(self.get_parameter("samples").value)
        self._sample_dt = float(self.get_parameter("sample_dt").value)
        self._max_mm = int(self.get_parameter("max_mm").value)

        cmd_topic = str(self.get_parameter("command_topic").value)
        result_topic = str(self.get_parameter("result_topic").value)
        self._pub = self.create_publisher(String, result_topic, 10)
        self._sub = self.create_subscription(String, cmd_topic, self._on_command, 10)

        self.get_logger().info("Sonar test node listo.")
        self._auto_timer = None
        if bool(self.get_parameter("auto_run").value):
            self._auto_timer = self.create_timer(1.0, self._auto_once)

    def _auto_once(self) -> None:
        if self._auto_timer is not None:
            self._auto_timer.cancel()
        self._run_test()

    def _publish_result(self, status: str, detail: str) -> None:
        msg = String()
        msg.data = f"sonar|{status}|{detail}"
        self._pub.publish(msg)

    def _on_command(self, msg: String) -> None:
        cmd = msg.data.strip().lower()
        if cmd in ("sonar", "all"):
            self._run_test()

    def _run_test(self) -> None:
        try:
            values = []
            for _ in range(self._samples):
                dist = self._sonar.get_distance_mm()
                values.append(dist)
                time.sleep(self._sample_dt)
            valid = [v for v in values if 0 < v <= self._max_mm]
            if not valid:
                self._publish_result("FAIL", f"muestras_invalidas:{values}")
                return
            avg = sum(valid) / float(len(valid))
            self._publish_result("PASS", f"avg_mm:{avg:.1f}")
        except Exception as exc:  # noqa: BLE001
            self._publish_result("FAIL", f"error:{exc!r}")

    def destroy_node(self) -> bool:
        try:
            self._sonar.shutdown()
        except Exception:  # noqa: BLE001
            pass
        return super().destroy_node()


def main(args=None) -> None:
    rclpy.init(args=args)
    node = FrankensteinSonarTestNode()
    try:
        rclpy.spin(node)
    finally:
        node.destroy_node()
        rclpy.shutdown()


if __name__ == "__main__":
    main()
